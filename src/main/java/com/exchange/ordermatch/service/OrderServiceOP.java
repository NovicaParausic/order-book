...
@Slf4j
@Service
public class OrderServiceOP extends OrderService {
	...
	private final LongAdaptiveRadixTreeMap<CurrencyPool> currencyPools; 
	
	private final boolean logDebug;

	public OrderServiceOP() {
//		this.symbolSpec = newCoreSymbolSpec();
...
//		//this.eventsHelper = new OrderBookEventsHelper(MatcherTradeEvent::new);
//		this.orderIdIndex = new LongAdaptiveRadixTreeMap<>(objectsPool);
		this.currencyPools = new LongAdaptiveRadixTreeMap<CurrencyPool>();
		this.logDebug = newLogginConfig();
	}

//	private CoreSymbolSpecification newCoreSymbolSpec() {
...
//		        .type(SymbolType.OPTION)//CURRENCY_EXCHANGE_PAIR)
...
//	}
	...
	private boolean newLogginConfig() {
		
		//LoggingConfiguration lgcfg = LoggingConfiguration.DEFAULT;
		LoggingConfiguration lgcfg = LoggingConfiguration.builder()
	            .loggingLevels(EnumSet.of(LoggingLevel.LOGGING_MATCHING_DEBUG))
	            .build();
		
		return lgcfg.getLoggingLevels()
				.contains(LoggingConfiguration.LoggingLevel.LOGGING_MATCHING_DEBUG);
	}

	
	@Override
    public void newOrder(final OrderCommand cmd, long currencyId) {

		final CurrencyPool currencyPool = currencyPools.get(currencyId);
		if (currencyPool == null) 
			return;
		
        switch (cmd.orderType) {
            case GTC:
                newOrderPlaceGtc(cmd, currencyPool);
                break;
            case IOC:
                newOrderMatchIoc(cmd, currencyPool);
                break;
            case FOK_BUDGET:
                newOrderMatchFokBudget(cmd, currencyPool);
                break;
          default:
	...
	@Override
    protected void newOrderPlaceGtc(final OrderCommand cmd, final CurrencyPool currencyPool) {
    	
        final long size = cmd.size;
        
        final LongAdaptiveRadixTreeMap<DirectOrder> orderIdIndex = currencyPool.getOrderIdIndex();
        final ObjectsPool objectsPool = currencyPool.getObjectsPool();
        
        // check if order is marketable there are matching orders
        final long filledSize = tryMatchInstantly(cmd, cmd, currencyPool);
        if (filledSize == size) {
        ...
        insertOrder(orderRecord, null, currencyPool.getSymobolId());
    }

	@Override
    protected void newOrderMatchIoc(final OrderCommand cmd, final CurrencyPool currencyPool) {
	...
	@Override
    protected void newOrderMatchFokBudget(final OrderCommand cmd, final CurrencyPool currencyPool) {

    	
        final long budget = checkBudgetToFill(cmd.action, cmd.size, currencyPool.getBestAskOrder(), currencyPool.getBestBidOrder());

        if (logDebug) log.debug("Budget calc: {} requested: {}", budget, cmd.price);

        if (isBudgetLimitSatisfied(cmd.action, budget, cmd.price)) {
            tryMatchInstantly(cmd, cmd, currencyPool);
        } else {
            //eventsHelper.attachRejectEvent(cmd, cmd.size);
        }
    }
    ...
    private long checkBudgetToFill(final OrderAction action, long size, 
    								final DirectOrder bestAskOrder, final DirectOrder bestBidOrder) {

    	//final CurrencyPool currencyPool = currencyPools.get(currencyId);
        DirectOrder makerOrder = (action == OrderAction.BID) 
        				? bestAskOrder
        				: bestBidOrder;
    ...
    private long tryMatchInstantly(final IOrder takerOrder,
                                   ...
                                   final CurrencyPool currencyPool) {
        ...
        if (isBidAction) {
            makerOrder = currencyPool.getBestAskOrder();
            if (makerOrder == null || makerOrder.price > limitPrice) {
                return takerOrder.getFilled();
            }
        } else {
            makerOrder = currencyPool.getBestBidOrder();
            if (makerOrder == null || makerOrder.price < limitPrice) {
                return takerOrder.getFilled();
            }
        }
...
//                    log.debug("  not completed, exit");
            ...
            if (makerOrder == priceBucketTail) {
                ...
                final LongAdaptiveRadixTreeMap<Bucket> buckets = isBidAction 
                		? currencyPool.getAskPriceBuckets()
                		: currencyPool.getBidPriceBuckets();
...
//        log.debug("makerOrder.parent = {}", makerOrder != null ? makerOrder.parent : null);
        ...
        if (isBidAction) {
        	currencyPool.setBestAskOrder(makerOrder);
        } else {
        	currencyPool.setBestBidOrder(makerOrder);
        }
    ...
    @Override
    public CommandResultCode cancelOrder(OrderCommand cmd, final long currencyId) {
        ...
        final Bucket freeBucket = removeOrder(order, currencyId);
        if (freeBucket != null) {
    ...
    @Override
    public CommandResultCode reduceOrder(OrderCommand cmd, final long currencyId) {
    ...
    @Override
    public CommandResultCode moveOrder(OrderCommand cmd, final long currencyId) {
    	...
    	ObjectsPool objectsPool = currencyPool.getObjectsPool();
    	LongAdaptiveRadixTreeMap<DirectOrder> orderIdIndex = currencyPool.getOrderIdIndex();
    	CoreSymbolSpecification symbolSpec = currencyPool.getSymbolSpec();
        ...
        // remove order
        final Bucket freeBucket = removeOrder(orderToMove, currencyId);
        ...
        final long filled = tryMatchInstantly(orderToMove, cmd, currencyPool);
        ...
        insertOrder(orderToMove, freeBucket, currencyId);

        return CommandResultCode.SUCCESS;
    }


    private Bucket removeOrder(final DirectOrder order, final long currencyId) {
		...
		final LongAdaptiveRadixTreeMap<Bucket> askPriceBuckets = currencyPool.getAskPriceBuckets();
		final LongAdaptiveRadixTreeMap<Bucket> bidPriceBuckets = currencyPool.getBidPriceBuckets();

		DirectOrder bestAskOrder = currencyPool.getBestAskOrder();
		DirectOrder bestBidOrder = currencyPool.getBestBidOrder();
    ...
    private void insertOrder(final DirectOrder order, final Bucket freeBucket, final long currencyId) {
...
//            log.debug(">>>> increment bucket {} from {} to {}", toBucket.tail.price, toBucket.volume, toBucket.volume +  order.size - order.filled);
    ...
    @Override
    public int getOrdersNum(OrderAction action, final long currencyId) {
    ...
    @Override
    public long getTotalOrdersVolume(OrderAction action, final long currencyId) {
    ...
    @Override
    public List<OrderDto> getEntries(final long currencyId) {
    	...
    	List<OrderDto> listAsk = extractList(askPriceBuckets.entriesList());
    	
    	for (OrderDto entry : listAsk) {
			System.out.println("Price: " + entry.getPrice() + ", Size: " + entry.getVolume());
		}
    	
    	System.out.println("bid price buckets");
    	List<OrderDto> listBid = extractList(bidPriceBuckets.entriesList()); 
    	
    	for (OrderDto entry : listBid) {
    		System.out.println("Price: " + entry.getPrice() + ", Size: " + entry.getVolume());
		}
	...
	private List<OrderDto> extractList(List<Map.Entry<Long, Bucket>> list) {
		List<OrderDto> ret = new ArrayList<>();
		
    	for (Map.Entry<Long, Bucket> entry : list) {
    		
    		OrderDto dto = new OrderDto(entry.getKey(), entry.getValue().volume, entry.getValue().numOrders);
    		ret.add(dto);
		}
...
//    }
    ...
    @Override
    public IOrder getOrderById(final long orderId, final long currencyId) {
...
//    @Override
//    public Stream<DirectOrder> askOrdersStream(boolean sortedIgnore) {
//        return null; //StreamSupport.stream(new OrdersSpliterator(bestAskOrder), false);
//    }

//    @Override
//    public Stream<DirectOrder> bidOrdersStream(boolean sortedIgnore) {
//        return null; //StreamSupport.stream(new OrdersSpliterator(bestBidOrder), false);
//    }
...
//    }
    ...
    @AllArgsConstructor
    @Builder
    public static final class DirectOrder implements IOrder {//WriteBytesMarshallable, 
